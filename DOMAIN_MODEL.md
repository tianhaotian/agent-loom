...
Schedule 是 tenant-scoped 定义实体，固定绑定 Workflow Version、Cron、timezone 和通用 Run 输入。每次触发把 `schedule_id` 与规范化 `scheduled_fire_at` 写入 Run，并以二元组唯一约束形成永久幂等边界；调度扫描不得仅依赖进程内定时器。
