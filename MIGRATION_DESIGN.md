...
| `context_snapshots` | immutable version | Run 通用 Context 与父级血缘 | `0017` |
...
### `0010_agent_invocation_envelope`

- 为 AgentExecution 增加不可为空的规范化 `request_json`；
- 与 ToolExecution 已有请求字段对齐，使 Server Adapter 可在 Worker 重启后重新装载同一调用；
- PostgreSQL 与 MySQL 使用各自 JSON 类型，但保持同一逻辑字段与非空约束。

### `0014_transactional_outbox`

- 创建 outbox_messages 和领取/回收索引；
- Event 与发布意图在同一事务创建，Publisher 使用短 Lease 和 attempt fencing。

### `0015_plan_revisions`

- 创建不可变 plan_revisions 历史；
...
### `0017_context_snapshots`

- 创建不可变 ContextSnapshot 与 ContextPatch 历史；
- Runs 增加当前 Context revision/ID 投影和同 Run 复合外键；
- 新 Run 原子写入 Snapshot 1，后续 Patch 使用 Run version 与 Context revision 双重 CAS，并保存父 Snapshot lineage。

### `0018_task_context_references`

- 为每个 Task 保存不可变 ContextSnapshot 引用和 JSON Pointer Projection；
- Task、Snapshot 与创建 Event 均使用同 Run 复合外键；
- 历史 Task 回填为 migration 时 Run 的当前 Snapshot，空 Projection 表示完整 Context。

每个批次结束运行 schema introspection 和最小插入/回滚 smoke test。基础 Provider 的 schema readiness 以所有必需 migration（当前为 `0000` 至 `0018`）成功且 Provider 黑盒测试通过为准。
...
3. 为 `0000`—`0018` 维护两套 migration 文件和 schema snapshot 测试。
