...
每个批次结束运行 schema introspection 和最小插入/回滚 smoke test。基础 Provider 的 schema readiness 以所有必需 migration（当前为 `0000` 至 `0017`）成功且 Provider 黑盒测试通过为准。
...
3. 为 `0000`—`0017` 维护两套 migration 文件和 schema snapshot 测试。
