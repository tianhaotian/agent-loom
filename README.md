...
Schedule V1 持久化五段 Cron、UTC timezone、绑定的 Workflow Version 和不可变 Run 输入。显式 fire API 使用 `(schedule_id, scheduled_fire_time)` 构造稳定幂等身份，Run 表以同一组合唯一约束防止重复触发；后台自动扫描、IANA timezone/DST、misfire 与 catch-up policy 在后续 P2 切片实现。
