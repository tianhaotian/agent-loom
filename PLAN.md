...
2. 将现有有界 Scheduler/Recovery Worker tick 接入常驻服务循环；补齐 Adapter 请求装载、真实调用结果回写、Lease 续租与回收。
