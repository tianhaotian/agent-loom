...
CI 已配置独立的 workspace 质量门禁和 PostgreSQL 16 服务 Job；后者通过 `AGENT_LOOM_TEST_POSTGRES_URL` 串行执行真实 migration 与事务垂直切片。后续按以下顺序推进：

1. 补充多 Worker 领取、续租/回收竞争与事务故障注入测试，并逐步把真实行为用例下沉为 Provider 黑盒 conformance harness。
