...
2. 将现有有界 Scheduler/Recovery Worker tick 接入常驻服务循环；使用 invocation 查询补齐真实 Adapter 调用、结果回写、Lease 续租与回收。
