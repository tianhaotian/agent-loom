...
2. 将现有有界 Scheduler tick 接入常驻服务循环，并实现 Worker 最小循环，把 Task 领取、Lease 续租/回收、恢复 Task 启动和 Adapter 调用串成可运行服务路径。
