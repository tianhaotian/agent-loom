...
2. 实现数据库权威时间驱动的通用过期 Lease 回收；长时普通 Task 使用续租，一次性 recovery Task 在启动事务内完成，不跨外部调用持有 Lease。
...
6. 按部署需求选择 `0011_optional_outbox`；将 `0012_runtime_grants` 作为数据库权限加固迁移或等价 IaC，而不是基础领域正确性的前置条件。
